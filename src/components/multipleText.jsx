import React, { useEffect } from "react";
import Typed from "typed.js";

import { LinearGradient } from "react-text-gradients";

const MultipleText = () => {
  useEffect(() => {
    // Initialize Typed.js on component mount
    const options = {
      strings: [
        "Software Engineer",
        "Full-Stack Developer",
        "Mobile Developer",
      ],
      typeSpeed: 100,
      backSpeed: 100,
      backDelay: 100,
      loop: true,
    };

    const typed = new Typed(".multiple-text", options);

    // Cleanup on component unmount
    return () => {
      typed.destroy();
    };
  }, []);

  return (
    <div className="flex flex-col items-center justify-center text-center p-4 poppins lg:mt-0 mt-20 sm:mt-32">
      <h4 className="text-white text-lg sm:text-xl md:text-2xl mb-2 font-semibold font-mono">Hello, It's Me</h4>
      <h1 className="text-white bbh-bartle text-3xl/[80%] sm:text-4xl/[80%] md:text-5xl/[80%] lg:text-6xl/[80%] xl:text-7xl/[80%] font-[800] mb-4 leading-tight">
        <LinearGradient gradient={["to left", "#2980B9 ,#6DD5FA"]}>
          KEVIN <br/> BRINSLY
        </LinearGradient>
      </h1>
      <h4 className="text-white text-lg sm:text-xl md:text-2xl mb-4 font-mono font-[700]">
        And I'm a{" "}
        <span className="multiple-text text-lg sm:text-xl md:text-2xl font-semibold text-[#17acff]"></span>
      </h4>
      {/* <p className="text-white text-xl max-w-xl mx-auto">
        
      </p> */}
    </div>
  );
};

export default MultipleText;
